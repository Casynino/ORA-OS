...
/**
 * Shared assembly for the credit & settlements workspace — used by both the
 * admin page and the finance department page (single source of truth).
 */
export async function getCreditWorkspaceData() {
  // Flip any past-due field credit to OVERDUE before reading — the credit
  // view must never show stale statuses.
  ...
  const creditAccounts: CreditAccountDTO[] = accounts.map((a) => {
    let running = 0;
  ...
  return { creditAccounts, settlements, fieldCredits, paymentAccounts };
}
